...
template<typename T, typename V>void debug(const map<T, V> &x) {ll comma = 0; cerr << '['; for (auto &i: x) cerr << (comma++ ? ", " : ""), debug(i); cerr << "]";}
template<typename T, typename V>void debug(const unordered_map<T, V> &x) {ll comma = 0; cerr << '['; for (auto &i: x) cerr << (comma++ ? ", " : ""), debug(i); cerr << "]";}
template <typename ST>void debug(stack<ST> st) {ll comma = 0; cerr << "["; while (!st.empty()) { cerr << comma++ ? ", " : ""; debug(st.top()); st.pop(); } cerr << "]";}
template<typename Q> void debug(queue<Q> que) {ll comma = 0; cerr << "["; while (!que.empty()) { cerr << comma++ ? ", " : ""; debug(que.front()); que.pop(); } cerr << "]";}
template<typename Q> void debug(priority_queue<Q> pq) {ll comma = 0; cerr << "["; while (!pq.empty()) { cerr << comma++ ? ", " : ""; debug(pq.top()); pq.pop(); } cerr << "]";}
template<typename DQ> void debug(deque<DQ> dque) {ll comma = 0; cerr << "["; while (!dque.empty()) { cerr << comma++ ? ", " : ""; debug(dque.front()); dque.pop_front(); } cerr << "]";}
